...
class AzurePricingServer:
    ...
    async def _make_request(
        self, url: str, params: Dict[str, Any] = None, max_retries: int = 3
    ) -> Dict[str, Any]:
    ...
    async def search_azure_prices(
        ...
        if validate_sku and sku_name and not items:
            validation_info = await self._validate_and_suggest_skus(
                service_name, sku_name, currency_code
            )
            ...
            validation_info["clarification"] = {
                ...
                "suggestions": [
                    item.get("skuName") for item in items[:5] if item and item.get("skuName")
                ],
            }
    ...
    async def _validate_and_suggest_skus(
        self, service_name: Optional[str], sku_name: str, currency_code: str = "USD"
    ) -> Dict[str, Any]:
        ...
        return {
            "sku_validation": {
                ...
                "found": False,
                "message": f"SKU '{sku_name}' not found"
                + (f" in service '{service_name}'" if service_name else ""),
    ...
    def _apply_discount_to_items(self, items: List[Dict], discount_percentage: float) -> List[Dict]:
        ...
        for item in items:
            ...
            if (
                "savingsPlan" in item
                and item["savingsPlan"]
                and isinstance(item["savingsPlan"], list)
            ):
                discounted_savings = []
    ...
    async def compare_prices(
        ...
        if regions and isinstance(regions, list):
            ...
            result = await self.search_azure_prices(
                service_name=service_name, currency_code=currency_code, limit=20
            )
    ...
    async def estimate_costs(
        ...
        for plan in savings_plans:
            ...
            savings_percent = (
                ((hourly_rate - plan_hourly) / hourly_rate) * 100 if hourly_rate > 0 else 0
            )
    ...
    async def _find_similar_services(
        ...
        if search_term in service_mappings:
            ...
            result = await self.search_azure_prices(
                service_name=correct_name, currency_code=currency_code, limit=limit
            )
        ...
        for azure_service in list(set(partial_matches)):
            result = await self.search_azure_prices(
                service_name=azure_service, currency_code=currency_code, limit=5
            )
        ...
        if result["items"]:
            ...
            for sku_data in skus.values():
                ...
                if valid_prices:
                    ...
                    sku_data["min_price"] = (
                        sku_data["prices"][0]["price"] if sku_data["prices"] else 0
                    )
                sku_data["sample_unit"] = (
                    sku_data["prices"][0]["unit"] if sku_data["prices"] else "Unknown"
                )
                    ...
                    if result["count"] > 0:
                        ...
                        if "discount_applied" in result:
                            ...
                            if total_savings > 0:
                                ...
                                response_text += (
                                    f"   Discounted Total: ${total_discounted_cost:.6f}\n"
                                )
...
"""
                ...
                if (
                    "discount_applied" in result
                    and "original_hourly_rate" in result["on_demand_pricing"]
                ):
                    estimate_text += f"""
...
"""
                ...
                if skus:
                    ...
                    return [
                        TextContent(type="text", text="No SKUs found for the specified service.")
                    ]
                ...
                if result["service_found"]:
                    ...
                    for product, product_skus in products.items():
                        ...
                        for sku_name, sku_data in sorted(product_skus)[
                            :10
