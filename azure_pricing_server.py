...
class AzurePricingServer:
    ...
    async def discover_skus(
        self, service_name: str, region: Optional[str] = None, price_type: str = "Consumption", limit: int = 100
    ) -> Dict[str, Any]:
    ...
    async def discover_service_skus(
        self, service_hint: str, region: Optional[str] = None, currency_code: str = "USD", limit: int = 30
    ) -> Dict[str, Any]:
        """
                ...
                if result["items"]:
                    ...
                    if result["count"] > 0:
                        ...
                        if "discount_applied" in result:
                            response_text += (
                                f"💰 **Customer Discount Applied: {result['discount_applied']['percentage']}%**\n"
                            )
